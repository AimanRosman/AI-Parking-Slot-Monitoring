...
from ultralytics.solutions.solutions import LOGGER, BaseSolution, SolutionAnnotator as Annotator, check_requirements
...
class ParkingManagement(BaseSolution):
    ...
    def __init__(self, **kwargs):
        ...
        # Timer properties
        self.spot_timers = [0] * len(self.json)
        self.spot_occupied_since = [None] * len(self.json)
    ...
    def process_data(self, im0):
        ...
        for region in self.json:
            ...
            # Update timers
            idx = self.json.index(region)
            if rg_occupied:
                if self.spot_occupied_since[idx] is None:
                    self.spot_occupied_since[idx] = time.time()
                self.spot_timers[idx] = int(time.time() - self.spot_occupied_since[idx])
            else:
                self.spot_occupied_since[idx] = None
                self.spot_timers[idx] = 0

            fs, es = (fs + 1, es - 1) if rg_occupied else (fs, es)
            # Plotting regions
            cv2.polylines(im0, [pts_array], isClosed=True, color=self.occ if rg_occupied else self.arc, thickness=2)
            
            # Display timer if occupied
            if rg_occupied:
                duration = self.spot_timers[idx]
                min, sec = divmod(duration, 60)
                time_str = f"{min:02d}:{sec:02d}"
                # Get center of region to display timer
                avg_pt = pts_array.mean(axis=0).flatten()
                cv2.putText(im0, time_str, (int(avg_pt[0]-25), int(avg_pt[1])), 
